...
function divider(numbersArray){
  ...
  for(x in numbersArray){
    if(numbersArray[x]%2===0){
      newArr[0].push(numbersArray[x])
    } else if(!numbersArray[x]%2===0) {
      newArr[1].push(numbersArray[x])
    }
  }
  return newArr
}
...
function removeItem(groceryList,otheritem){
if(groceryList && otheritem){for(i=0;i<groceryList.length;i++){
    if(groceryList[i]===otheritem){
      groceryList.splice(i,1)
      }

  } return groceryList} else {
    return [];
  }
}
